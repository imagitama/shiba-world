...
const slimSpecies = (docData) => ({
  ...
  [SpeciesFieldNames.shortDescription]:
    docData[SpeciesFieldNames.shortDescription],
...
const tallyAvatarsForSpecies = (species, avatarsByPageNumber) => {
  ...
  for (const [, avatars] of Object.entries(avatarsByPageNumber)) {
    for (const avatarDoc of avatars) {
      tally[avatarDoc.speciesId] =
        avatarDoc.speciesId in tally ? tally[avatarDoc.speciesId] + 1 : 1
    }
  }
...
const getSpeciesNameForId = (speciesId, species) =>
  species.find(({ id }) => id === speciesId)[SpeciesFieldNames.pluralName]
...
const syncAvatarPages = async () => {
  ...
  const sortedAssets = sortAvatarsBySpeciesName(
    avatarsWithSpeciesDupes,
    species
  )
