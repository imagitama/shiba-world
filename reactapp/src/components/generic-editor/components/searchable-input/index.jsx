...
const getDocIdFromValueOrRef = valueOrRef => {
  if (isRef(valueOrRef)) {
    return valueOrRef.ref.id
  } else {
    return valueOrRef.id
  }
}
...
export default ({ name, onChange, value, fieldProperties }) => {
  ...
  useEffect(() => {
    ...
    const docId = getDocIdFromValueOrRef(value)

    async function main() {
      console.debug('populating', fieldProperties.collectionName, docId)
      setValueData({
        id: docId,
        ...mapDates(
          await quickReadRecord(fieldProperties.collectionName, docId)
        )
      })
    }
