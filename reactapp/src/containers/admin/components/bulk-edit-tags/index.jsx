...
const AssetsOutput = ({ tagToSearch, onConfirmWithIds }) => {
  let [isLoading, isErrored, assets] = useDatabaseQuery(
    ...
    tagToSearch ? [[AssetFieldNames.tags, Operators.ARRAY_CONTAINS, tagToSearch]] : false,
    { [options.populateRefs]: false, [options.subscribe]: false }
  )
  ...
  return (
    <>
      ...
      <Button onClick={() => onConfirmWithIds(assets.map(({ id }) => id))}>Yes - next step</Button>
    </>
  )
}
    ...
    case Steps.Search:
      return (
        <>
          ...
          <TextInput value={tagToSearch} onChange={e => setTagToSearch(e.target.value)} />
          <Button onClick={() => setStepIdx(Steps.ConfirmAssets)}>Perform Search</Button>
        </>
      )
      ...
      case Steps.ConfirmAssets:
        return (
          <>
            ...
            <p>
              The operation will be applied to these assets.
            </p>
            <AssetsOutput tagToSearch={tagToSearch} onConfirmWithIds={onConfirmWithIds} />
                  <Button onClick={() => {
                    setTagToSearch(null)
                    setStepIdx(Steps.Search)
                  }} color="default">No - return to search</Button>
          </>
        )
      ...
      case Steps.Operation:
        return (
          <>
            ...
            <br />
            <>Choose the tag: <TextInput value={tag} onChange={e => setTag(e.target.value)} /><br /></>
            {operationName === OperationNames.Rename && <>Choose a new tag to rename to: <TextInput value={additionalTag} onChange={e => setAdditionalTag(e.target.value)} /><br /></>}
            <Button onClick={() => setStepIdx(Steps.ConfirmOperation)}>Confirm</Button>
          </>
        )
      ...
      case Steps.ConfirmOperation:
        return (
          <>
            ...
            <p>
              Confirm the bulk editing operation:
            </p>
            <strong>{assetIds.length} assets</strong> perform operation <strong>{operationName}</strong> with tag <strong>{tag}</strong> {additionalTag ? <>to <strong>{additionalTag}</strong></> : null}
            <p>
              <span style={{ color: 'red' }}>Warning this operation cannot be undone!</span>
            </p>
            <p>
              <span style={{ color: 'orange' }}>Warning this operation will trigger side-effects for each asset modified (for example it might send a tweet or notification or something)</span>
            </p>
            <p>
              <span style={{ color: 'orange' }}>Note that your name will be set as the person who last modified each asset</span>
            </p>
